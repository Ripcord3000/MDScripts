...
/**
 ...
 Author MD
 */
public class FastCreatePlanet extends UserDefinedCommand {
	private String name;
	...
	public void onText(String buffer, String text) throws Exception {
		...
		if ((range = ((Tools.findText(buffer, "What do you want to name this planet? (",")")))) != null) {
			...
			if(Swath.sector.armidMines() > 0 && !(Swath.sector.armidOwner().isYou() || Swath.sector.armidOwner().isYourCorporation())){
		...
		else if(((range = ((Tools.findText(buffer, "<Atmospheric maneuvering system engaged>","Landing sequence engaged...")))) != null)){
			setBufferText("");
			sendString("/zdy");
			if(Swath.sector.armidMines() > 0 && !(Swath.sector.armidOwner().isYou() || Swath.sector.armidOwner().isYourCorporation())){
				sendString(RETURN_KEY);
...
//		printTrace("onEvent('" + event.getClassName() + "')");
	...
	public static boolean exec(String name, PlanetClass[] c, int owner, String nameIfNotRight) throws Exception	{
		...
		FastCreatePlanet cmd = new FastCreatePlanet();
		cmd.initInstance();
		cmd.c = c;
		cmd.name = name;
		cmd.owner = owner;
